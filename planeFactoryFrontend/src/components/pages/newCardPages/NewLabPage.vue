  ...
  methods: {
    async saveData() {
      ...
      if (result.data.status == 'OK') {
        ...
        this.$store.commit('setLabs', result.data.labs)
        this.$router.push('/labs/' + this.$store.getters.labs[0].ID)
      } else {
        alarm('Не удалось создать лабораторию')
      }
    }
  }
}
