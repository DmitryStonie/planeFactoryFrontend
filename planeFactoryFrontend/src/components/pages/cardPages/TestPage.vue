<template>
  ...
  <div class="mx-auto max-w-3xl px-6 bg-white border-0 shadow-lg sm:rounded-3xl">
    <div class="relative z-0 w-full mb-4 mt-10">
      <input
        type="text"
        v-model="$store.getters.tests[0].Name"
        name="name"
        rows="2"
        placeholder=" "
        required
    ...
    <p class="text-xl mb-6">Id: {{ $store.getters.tests[0].ID }}</p>
    <form class="mx-auto">
      ...
      <textarea
        id="message"
        v-model="$store.getters.tests[0].Description"
        rows="8"
    ...
    <div class="relative z-0 w-full mb-5">
      <input type="number" v-model="$store.getters.tests[0].Product" list="product" class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none z-1 focus:outline-none focus:ring-0 focus:border-black border-gray-200"/>
      <datalist id="product">
    ...
    <div class="relative z-0 w-full mb-5">
      <select
        name="select"
        v-model="$store.getters.tests[0].Lab"
    ...
    <div class="flex flex-row space-x-4">
      <div class="relative z-0 w-full mb-5">
        <input
          type="date"
          v-model="$store.getters.tests[0].TimeStart"
          name="date"
          placeholder=" "
    ...
    <div class="flex flex-row space-x-4">
      <div class="relative z-0 w-full mb-5">
        <input
          type="date"
          v-model="$store.getters.tests[0].TimeEnd"
          name="date"
          placeholder=" "
          onclick="this.setAttribute('type', 'date');"
          class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"
...
export default {
  ...
  methods: {
    async saveData() {
      this.$store.dispatch('putTest', this.$route.params.id)
    },
    async deleteData(){
      await this.$store.dispatch('deleteTest', this.$route.params.id)
      this.$router.go(-1)
    }
  }
}
