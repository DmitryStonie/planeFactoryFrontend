<template>
  ...
  <div class="mx-auto max-w-3xl px-6 bg-white border-0 shadow-lg sm:rounded-3xl">
    <div class="relative z-0 w-full mb-4 mt-10">
      <input
        type="text"
        v-model="$store.getters.areas[0].Name"
        name="name"
        rows="2"
        placeholder=" "
        required
    ...
    <div class="relative z-0 w-full mb-5">
      <select
        name="select"
        v-model="$store.getters.areas[0].Workshop"
    ...
    <div class="relative z-0 w-full mb-5">
      <select
        name="select"
        v-model="$store.getters.areas[0].Boss"
        onclick="this.setAttribute('value', this.value);"
        class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none z-1 focus:outline-none focus:ring-0 focus:border-black border-gray-200"
      >
    ...
    <div class="flex flex-wrap items-center justify-between gap-5 w-full mt-8 mb-4">
      <h1 class="text-2xl font-bold ml-4">Brigades</h1>
