          ...
          <div class="grid grid-rows-1 gap-6 md:grid-rows-2 lg:grid-rows-3">
            <div class="flex flex-col">
              ...
              <select
                ...
                v-model="$store.getters.FilterProps.selected.category3"
              >
                <option
            ...
            <div class="flex flex-col">
              ...
              <input
                ...
                v-model="$store.getters.FilterProps.selected.dateFrom3"
              />
            </div>
            ...
            <div class="flex flex-col">
              ...
              <input
                ...
                id="date"
                class="mt-2 block w-full cursor-pointer rounded-md border border-gray-100 bg-gray-100 px-2 py-2 shadow-sm outline-none focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                v-model="$store.getters.FilterProps.selected.dateTo3"
              />
            </div>
          </div>
...
export default {
  methods: {
    resetParams() {
      this.$store.commit('cleanCategory3F')
      this.$store.commit('cleanDateFrom3F')
      this.$store.commit('cleanDateTo3F')
    },
    search() {
      this.$store.dispatch('getLab', {
        id: this.$route.params.id,
        payload: {
          category: this.$store.getters.FilterProps.selected.category3,
          datefrom: this.$store.getters.FilterProps.selected.dateFrom3,
          dateto: this.$store.getters.FilterProps.selected.dateTo3
        }
      })
    }
  }
