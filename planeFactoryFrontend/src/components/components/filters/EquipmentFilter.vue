          ...
          <div class="grid grid-rows-1 gap-6 md:grid-rows-2 lg:grid-rows-3">
            <div class="flex flex-col">
              ...
              <select
                ...
                v-model="$store.getters.FilterProps.selected.category2"
              >
                <option
...
export default {
  methods: {
    resetParams() {
      this.$store.commit('cleanCategory2F')
      this.$store.commit('cleanDateFrom2F')
      this.$store.commit('cleanDateTo2F')
    },
    search() {
      this.$store.dispatch('getLab', {
        id: this.$route.params.id,
        payload: {
          category: this.$store.getters.FilterProps.selected.category2,
          datefrom: this.$store.getters.FilterProps.selected.dateFrom2,
          dateto: this.$store.getters.FilterProps.selected.dateTo2
        }
      })
    }
  }
